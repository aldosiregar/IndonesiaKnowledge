 ...
 */
function fetchAPI(location, type, option){
    if(type == "materials"){
        const result = fetch(location).then(res => {
            if(!res.ok){
                throw new Error(`HTTP status error : ${res.status}`);
            }
            return res.json();
        }).then((data) => {
            return materialsFetcher(data, option);
        }).catch((err) =>{
...
function materialsFetcher(data, option){
    let content = "";
    content += "<h1 class='text-center'>" + data["Bab_" + option].Judul + "</h1>";
